...
const Books = () => {
    ...
    const ReadBook=async(item)=>{
        try{
            const res=await axios.post(`${import.meta.env.VITE_BACKEND_API || "http://localhost:8080"}/api/user/purchase`,{item});
        } catch(error){
    ...
    <div>
      ...
      <div className="bg-gray-100 h-screen flex flex-row flex-wrap gap-8 text-black pt-10">
        {[
          ...
          return (
            <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} className="h-50 w-80 bg-white rounded-xl shadow-2xl hover:h-52 hover:w-82 transition-all items-center flex flex-col justify-center">
